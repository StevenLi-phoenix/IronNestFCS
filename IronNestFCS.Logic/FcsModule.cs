...
public class FcsModule : IFcsModule
{
    ...
    public void CrateCustomRecorder() {
    ...
    private static void ExportTexture(MeshRenderer renderer, string fileName) {
        ...
        if (tex == null) {
            MelonLogger.Error("[FCS] 导出失败：material.mainTexture 为空");
            return;
        }
        ...
        try {
            ...
            MelonLogger.Msg($"[FCS] 已导出贴图 {w}x{h} → {path}");
        }
        catch (Exception ex) {
            MelonLogger.Error($"[FCS] 导出贴图失败: {ex}");
        }
        finally {
    ...
    private static void ReplaceTexture(MeshRenderer renderer, string fileName) {
        ...
        if (!System.IO.File.Exists(path)) {
            MelonLogger.Error($"[FCS] 找不到贴图文件: {path}");
            return;
        }
        ...
        try {
            ...
            if (!ImageConversion.LoadImage(tex, new Il2CppStructArray<byte>(bytes), false)) {
                MelonLogger.Error($"[FCS] LoadImage 解析失败: {path}");
                Object.Destroy(tex);
                return;
            }
            // 用实例 material（非 sharedMaterial）替换，避免改到其他共享此材质的物体。
            renderer.material.mainTexture = tex;
            MelonLogger.Msg($"[FCS] 已替换贴图 {tex.width}x{tex.height} ← {path}");
        }
        catch (Exception ex) {
            MelonLogger.Error($"[FCS] 替换贴图失败: {ex}");
        }
    }
    ...
    private void LoadCustomTrack(RecordItem recordItem) {
        ...
        if (!System.IO.File.Exists(path)) {
            MelonLogger.Error($"[FCS] 找不到音频文件: {path}");
            return;
        }
        ...
        try {
            ...
            MelonLogger.Error($"[FCS] 解析 a.wav 失败: {ex}");
            return;
        }
        ...
        MelonLogger.Msg($"[FCS] 已替换 RecordItem.tracks，时长 {(float)lengthSamples / sampleRate:F1}s");
    }
